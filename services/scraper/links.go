...
func (s *scraperService) linksToCrawl(ctx context.Context, content string, workspaceDomains []string) ([]string, error) {
	spans, ctx := telemetry.StartServiceSpan(ctx, "scraperService.linksToCrawl")
	defer spans.Finish()
	...
	for _, webpage := range webpages {
		for _, link := range webpage.Links {
			if shouldCrawl(link, workspaceDomains) {
...
func extractLinks(linksSection string) []string {
	var urls []string
...
func shouldCrawl(url string, workspaceDomains []string) bool {
